...
export class ExportRecieptPrintComponent implements OnInit {
  ...
  CustomerName:string;
  constructor(private rout : ActivatedRoute,private reciept:ExportRecieptService,private CustSer:CustService , private prodsServ : ProductService ) {
  ...
  ngOnInit(): void {
    ...
    this.reciept.getRecieptsByID(parseInt(this.recieptID)).subscribe(Data => {
      ...
      this.CustSer.getCustomerByID(this.ExportReciept.customerID).subscribe(Date => {
        this.CustomerName = Date.name
      });
      this.ExportReciept.products.forEach(element => {
        this.prodsServ.getProductsByID(<number>element.productID).subscribe(Data => {
          this.ExportReciept.products.find( pro => pro.productID == element.productID).productName = Data.name;
        })
      });
