...
export class RecieptPrintComponent implements OnInit {
  ...
  SuppLierName:string;
  constructor(private rout : ActivatedRoute,private reciept:ImportReceiptService,private suppSer:SupplierService , private prodsServ : ProductService ) {
  ...
  ngOnInit(): void {
    ...
    this.reciept.getRecieptsByID(parseInt(this.recieptID)).subscribe(Data => {
      ...
      this.suppSer.getSupplierByID(this.ImportReciept.supid).subscribe(Date => {
      ...
      this.ImportReciept.importProducts.forEach(element => {
        this.prodsServ.getProductsByID(element.productID).subscribe(Data => {
          this.ImportReciept.importProducts.find( pro => pro.productID == element.productID).productName = Data.name;
        })
      });
