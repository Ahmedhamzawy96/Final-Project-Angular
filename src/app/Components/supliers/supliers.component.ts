...
export class SupliersComponent implements OnInit {
  ...
  addsupp: ISupplier[]=[];
  constructor(private _supplierservice: SupplierService ,private fBuilder:FormBuilder   ) {
...
notes:['']
  ...
  addsupplier(){
    this._supplierservice.addSupplier(this.userSupplier.value) .subscribe((result:ISupplier)=>
    {
  ...
  swalWithBootstrapButtons.fire({
   ...
   swalWithBootstrapButtons.fire(
      ...
      result.dismiss === Swall.DismissReason.cancel
    ) {
      swalWithBootstrapButtons.fire(
        'الغاء',
        'لم يتم حذف المورد ',
